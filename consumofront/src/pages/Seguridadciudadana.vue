<template>
  <q-page class="q-pa-xs">
    <q-card>
      <q-badge class="text-h6 full-width text-center" color="positive" >Aprobar por seguridad ciudadana</q-badge>
...
export default {
  ...
  methods:{
    daralta(i){
      ...
      this.$q.dialog({
        ...
        this.$axios.put(process.env.API+'/direccion/'+id,{
          estado:'PROCESO',
          nombre:'APROBADO SEGURIDAD CIUDADANA',
    ...
    mistramites(){
      this.$q.loading.show()
      this.$axios.get(process.env.API+'/direccion/s').then(res=>{
        // console.log(res.data)
        this.tramites=[]
        res.data.forEach(r=>{
